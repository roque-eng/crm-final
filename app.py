...
with tab3:
    ...
    with st.container(border=True):
        ...
        cont_cot = c_con.text_input("Nombre y Contacto")
    ...
    if st.button("💾 Guardar y Ver", key="sv_ind"):
        ...
        if guardar_en_db(db_data):
            st.success("Guardado"); st.link_button("👉 VER VISTA PREVIA", l_final)
...
with tab_flota:
    ...
    # RESTAURADAS COLUMNAS DE DEDUCIBLE
    ...
    if st.button("💾 Guardar y Ver Flota", key="sv_fl"):
        datos_f = {"n": f_nom, "e": f_ase, "cont": f_cont, "tab": t_flota.to_dict(orient='records'), "ben": "Auxilio 24hs incluido."}
        ...
        if guardar_en_db(db_data):
            st.success("Guardado"); st.link_button("👉 VER VISTA PREVIA", l_f)
...
with tab3:
            ...
            if not match.empty:
                for col_name in ["Asegurado", "Asegurado (Nombre/Razón Social)", "Cliente"]:
                    if col_name in df_raw.columns:
                        nombre_sug = match.iloc[0][col_name]; break
    ...
    with col_b:
        ...
        txt_a = "• Auto cortesía 15 días\nCosto Anual: UYU 3.500"; txt_b = "• Hurto USD 1.000\nCosto Anual: USD 110"
    ...
    if st.button("💾 Guardar y Ver Vista Previa", key="btn_ind"):
        ...
        if guardar_en_db(db_data):
            ...
            st.markdown(f"**Link generado:** {l_final}")
            st.components.v1.html(f'<script>window.open("{l_final}", "_blank");</script>', height=0)
            st.link_button("👉 Abrir Vista Previa (Si el navegador bloqueó)", l_final)
        else: st.error("Error al guardar en Supabase.")
...
with tab_flota:
    ...
    with st.container(border=True):
        ...
        f_ase = f5.selectbox("Asesor Flota", sorted(list(USUARIOS.keys())), key="ase_flota_sel"); f_cont = f6.text_input("Contacto Asesor", key="c_flota_t")
    t_flota = st.data_editor(pd.DataFrame([{"Vehículo": "Unidad 1", f"Precio {f_as1}": 0, f"Precio {f_as2}": 0, f"Precio {f_as3}": 0}]), num_rows="dynamic", use_container_width=True)
    if st.button("💾 Guardar y Ver Vista Previa", key="btn_flota"):
        ...
        db_data = {"tipo": "flota", "asegurado": f_nom, "vehiculo_o_flota": "Flota", "asesor": f_ase, "datos_json": datos_f, "link_cotizacion": l_f}
        if guardar_en_db(db_data):
            st.success("¡Flota guardada en Historial!")
            st.markdown(f"**Link generado:** {l_f}")
            st.components.v1.html(f'<script>window.open("{l_f}", "_blank");</script>', height=0)
            st.link_button("👉 Abrir Vista Previa (Si el navegador bloqueó)", l_f)
        else: st.error("Error al guardar en Supabase.")
