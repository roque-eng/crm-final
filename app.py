...
st.markdown("""
    <style>
    /* Título alineado a la izquierda con aire superior */
    .left-title {
    ...
    /* Evitar que el encabezado se corte */
    .block-container {
        padding-top: 2.5rem !important;
    }
    /* Color de fondo suave para los buscadores */
...
with col_user_status:
    # Espaciador para alinear con el título
...
# ---------------- PESTAÑA 2: PÓLIZAS (Buscador y Filtros) ----------------
with tab2:
    col_pol_h, col_pol_s = st.columns([2, 1])
    col_pol_h.subheader("📂 Pólizas Vigentes")
    busqueda_pol = col_pol_s.text_input("🔍 Buscar póliza...", placeholder="Nombre o CI del asegurado", key="search_pol")
    ...
    if not df_p.empty:
        ...
        st.info("No se encontraron pólizas.")
...
with tab3:
    ...
    sql_v = f"SELECT c.nombre_completo as 'Cliente', c.celular, s.aseguradora, s.ramo, s.ejecutivo, TO_CHAR(s.vigencia_hasta, 'DD/MM/YYYY') as 'Vence' FROM seguros s JOIN clientes c ON s.cliente_id = c.id WHERE {' AND '.join(cond_v)} ORDER BY s.vigencia_hasta ASC"
