...
# ==========================================
# ⚙️ CARGA Y PROCESAMIENTO (SIMPLE Y ROBUSTO)
# ==========================================
...
# --- CONFIGURACIÓN MÍNIMA (SOLO ICONOS) ---
config_simple = {}
if "Adjunto (póliza)" in df_f.columns:
    config_simple["Adjunto (póliza)"] = st.column_config.LinkColumn("Póliza", display_text="📂")
if "Premio_Total_USD" in df_f.columns:
    config_simple["Premio_Total_USD"] = st.column_config.NumberColumn("Total USD", format="U$S %d")
...
with tab2:
    ...
    if not df_f.empty and "Fin de Vigencia" in df_f.columns:
        ...
        # Filtro de seguridad para años (2020 a 2040)
        ...
        st.info("No se encontró la columna 'Fin de Vigencia' o no hay datos.")

# --- TAB 3: COTIZADOR (DETALLES AMPLIOS) ---
with tab3:
...
with tab4:
    if not df_f.empty:
        ...
        with c_g1:
            st.plotly_chart(px.pie(df_f, names='Aseguradora' if 'Aseguradora' in df_f.columns else df_f.columns[0], values='Premio_Total_USD', title="Cartera por Cía", hole=0.4), use_container_width=True)
        with c_g2:
